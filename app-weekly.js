...
function wWeek(v){return Math.min(18,Math.max(1,Math.trunc(Number(v)||1)))}
function wPts(s,k){return Number(s?.[k]||0)+Number(s?.[k+"_decimal"]||0)/100}
function wRec(r){const n=Math.min(wCompleted(),Number(weeklyHQ.league?.settings?.playoff_week_start||19)-1);if(weeklyHQ.loaded&&Array.from({length:n},(_,i)=>weeklyHQ.weeks.has(i+1)).every(Boolean))return seasonRecord(r?.roster_id,Array.from({length:n},(_,i)=>weeklyHQ.weeks.get(i+1)));const s=r?.settings||{};return{w:Number(s.wins||0),l:Number(s.losses||0),t:Number(s.ties||0),pf:wPts(s,"fpts"),pa:wPts(s,"fpts_against")}}
...
function wStandings(){
  return weeklyHQ.rosters.map(r=>({r,m:wManager(r),x:wRec(r)})).sort((a,b)=>recordRate(b.x)-recordRate(a.x)||b.x.pf-a.x.pf||a.m.team.localeCompare(b.m.team));
}
...
async function wBasics(force){
  ...
  const followed=weeklyHQ.selected==null||weeklyHQ.selected===season.current;
  weeklyHQ.nfl=nfl;weeklyHQ.league=league;weeklyHQ.rosters=rosters||[];weeklyHQ.users=users||[];weeklyHQ.loaded=true;
  await seasonSync(nfl,league,force);
  if(followed)weeklyHQ.selected=season.current;
}
async function wLoadWeek(week,force){
  ...
  const normalized=(rows||[]).map(r=>({...r,points:r.custom_points??r.points}));weeklyHQ.weeks.set(week,normalized);return normalized;
}
function wCompleted(){return Math.min(weeklyHQ.selected||season.current,season.completed)}
...
function wPower(history){
  ...
  return weeklyHQ.rosters.map(r=>{
    const id=Number(r.roster_id),x=seasonRecord(id,history),games=x.w+x.l+x.t,winPct=games?(x.w+.5*x.t)/games:.5;
...
function wKpis(rows){
  const current=season.current, standings=wStandings(), leader=standings[0], groups=wGroups(rows);
  ...
  $("#weeklyKpis").innerHTML=[
    ["WEEK",weeklyHQ.selected,seasonMode(weeklyHQ.selected)],
    ...
    ["SCOREBOARD",seasonMode(weeklyHQ.selected),scored?(scored+"/12 scoring"):"Sleeper sync"]
...
function wTeam(row){
  if(!row)return '<div class="weekly-empty">Bye / opponent pending</div>';
...
function wMatchups(rows){
  const current=season.current,final=seasonMode(weeklyHQ.selected)==="FINAL",groups=wGroups(rows),el=$("#weeklyMatchups");
  ...
  el.innerHTML=groups.map(g=>{
    const a=g.teams[0],b=g.teams[1],ap=Number(a?.points||0),bp=Number(b?.points||0),diff=Math.abs(ap-bp),state=seasonMode(weeklyHQ.selected);
...
function wAwards(rows,power,week=weeklyHQ.selected){
  const current=season.current,final=seasonMode(week)==="FINAL",groups=wGroups(rows).filter(g=>g.teams.length>1),scored=(rows||[]).filter(r=>final?r.points!=null:Number(r.points||0)>0);
  if(!scored.length){
...
function wPowerBoard(power){
  ...
  $("#weeklyPower").innerHTML=power.map((z,i)=>{
    ...
    return'<article class="power-row"><div class="power-rank">'+(i+1)+'<small class="rank-move">'+(n<2?'NEW':rankMovement(previous,z.r.roster_id,i))+'</small></div><img src="'+esc(z.m.avatar)+'" alt=""><div class="power-team"><strong>'+esc(z.m.team)+'</strong><span>'+recordText(z.x)+' · '+z.x.pf.toFixed(1)+' PF</span></div><div class="power-metric"><small>INDEX</small><strong>'+(n?z.power.toFixed(0):"—")+'</strong></div><div class="power-metric"><small>ALL-PLAY</small><strong>'+(n?(z.all*100).toFixed(0)+"%":"—")+'</strong></div><div class="power-metric luck '+(z.luck>.35?"lucky":z.luck<-.35?"unlucky":"")+'"><small>LUCK</small><strong>'+(n?luck:"—")+'</strong></div></article>';
  }).join("");
...
function wPlayoff(){
  ...
  $("#weeklyPlayoff").innerHTML='<div class="playoff-column"><div class="playoff-column-title">CURRENTLY IN</div>'+inside.map((z,i)=>team(z,i+1)).join("")+'</div><div class="playoff-column bubble-column"><div class="playoff-column-title">CHASING THE LINE</div>'+bubble.map((z,i)=>team(z,spots+i+1)).join("")+'<div class="playoff-note">Display tiebreak: win percentage (ties count half) → points for → team name. Sleeper remains the source of truth.</div></div>';
}

async function renderWeeklyHQ(force=false){
  if(weeklyHQ.loading){weeklyHQ.rerender=true;return;}weeklyHQ.loading=true;wLoading(force?"Refreshing every league board…":"Syncing all 12 teams from Sleeper…");
  try{
    await wBasics(force);wControls();const selected=weeklyHQ.selected;
    const [rows,history]=await Promise.all([wLoadWeek(weeklyHQ.selected,force),wHistory()]);
    if(selected!==weeklyHQ.selected){weeklyHQ.rerender=true;return;}
    const power=wPower(history);wKpis(rows);wMatchups(rows);wAwards(rows,power);wPowerBoard(power);wTable();wPlayoff();homeSeasonStyles();await seasonRender(rows,history,power);
    $("#weeklyUpdated").textContent="Updated "+new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})+" · Sleeper public API";wHide();
  }catch(e){
    console.error("Weekly HQ",e);$("#homeWeekEyebrow").textContent="SYNC FAILED · TRY WEEKLY HQ REFRESH";$("#homePulseFooter").textContent="Sleeper unavailable. Displayed results may be stale.";["#weeklyMatchups","#weeklyAwards","#weeklyPower","#weeklyStandings","#weeklyPlayoff"].forEach(s=>{const el=$(s);if(el)el.innerHTML='<div class="weekly-empty"><strong>Couldn’t load Sleeper.</strong><span>'+esc(e.message||"Try refresh.")+'</span></div>'});wHide();
  }finally{weeklyHQ.loading=false;if(weeklyHQ.rerender){weeklyHQ.rerender=false;renderWeeklyHQ();}}
}
function wMove(d){weeklyHQ.selected=wWeek((weeklyHQ.selected||season.current)+d);wControls();renderWeeklyHQ()}
